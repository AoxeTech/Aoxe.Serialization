...
public static partial class NewtonsoftJsonHelper
{
    ...
    public static TValue? FromStream<TValue>(Stream? stream, JsonSerializerSettings? settings = null,
        ...
        stream is null or { CanSeek: true, Length: 0 }
            ? default
            : (TValue?)FromStream(typeof(TValue), stream, settings, encoding ?? Defaults.Utf8Encoding);
        ...
        Encoding? encoding = null)
    {
        ...
        var result = FromJson(type, stream.ReadToEnd().GetString(encoding ?? Defaults.Utf8Encoding), settings);
