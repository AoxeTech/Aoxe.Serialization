...
public partial class MemoryPackHelper
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var result = await MemoryPackSerializer.DeserializeAsync<TValue>(stream, options, cancellationToken);
    ...
    {
        if (stream is null or { CanSeek: true, Length: 0 }) return default;
        var result = await MemoryPackSerializer.DeserializeAsync(type, stream, options, cancellationToken);
        stream.TrySeek(0, SeekOrigin.Begin);
        return result;
    }
}